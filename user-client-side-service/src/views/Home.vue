  ...
  <div class="flex justify-center py-6 px-3">
    <div class="lg:w-1/2 w-full bg-white rounded p-4 shadow">
      <div v-if="user" class="mb-2 flex justify-between items-center">
        <div>
          <h1 class="text-xl text-gray-600 flex items-center">
            Hello <b class="ml-1"> {{ user.firstName }}</b>

            <img
              v-if="['verified', 'VERIFIED'].includes(user.state.toUpperCase())"
              src="https://media.istockphoto.com/id/1396933001/vector/vector-blue-verified-badge.jpg?s=170667a&w=0&k=20&c=YTDqpWSoIXQS9Auw2Qor5g84eE2EqmLvJJmiHG7I83w="
              alt="verified badge"
              class="h-14 ml-1"
            />
          </h1>
          ...
          <span
            v-if="user.state"
            :class="
              `text-sm ${state.color} ${state.background} rounded px-3 py-1 font-semibold`
            "
            >{{ state.state.toUpperCase() }}</span
          >
        </div>
        ...
        <div class="relative" v-if="user.profilePicture">
          ...
          <span
            class="top-0 left-14 absolute  w-5 h-5 bg-green-400 border-2 border-white dark:border-gray-800 rounded-full"
          ></span>
        </div>
      ...
      <hr class="my-6 " />
      <user-details-form
        v-if="user && state.state == 'Unverified' && user.role === 'user'"
        @saved="loadUser"
      />
      <user
        v-if="user && state.state != 'Unverified' && user.role === 'user'"
